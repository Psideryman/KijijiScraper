...
while True:
    ...
    for listing in links:
        if (listing not in prevLinks):
            print('ENTER')
            pywhatkit.sendwhatmsg('+16135136708', ('https://www.kijiji.ca' + listing), datetime.datetime.now().hour, datetime.datetime.now().minute + 1)
            # pywhatkit.sendwhatmsg('+16135136708

            # ', listing, datetime.datetime.now().hour, datetime.datetime.now().minute + 1)
